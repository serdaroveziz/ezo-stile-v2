/* EZO STİLE v2 - Android Google Play Billing Sandbox Verification Endpoint */
...
export default async function handler(req, res) {
  ...
  try {
    const { userUid, purchaseToken, productId, businessId, isFakeToken } = req.body || {};

    if (!userUid || !purchaseToken || !productId) {
      return res.status(400).json({ error: 'userUid, purchaseToken ve productId zorunludur' });
    }

    // 1. FAKE TOKEN GUARD
    if (isFakeToken) {
      return res.status(400).json({ error: 'Sahte Google purchase token reddedildi.' });
    }

    // 2. IDEMPOTENCY GUARD (REPLAY PREVENTION)
    const tokenKey = 'google_' + crypto.createHash('md5').update(purchaseToken).digest('hex');
    ...
    if (existingPayment && (existingPayment.status === 'success' || existingPayment.status === 'refunded')) {
      return res.status(400).json({ error: 'Bu Google purchase token daha önce kullanılmıştır. İkinci işlem engellendi.' });
    }
    ...
    console.error('Google Play verification API error:', err);
